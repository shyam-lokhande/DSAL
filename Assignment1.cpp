...
Employee sm[SIZE];
public:
Directory(){ for(int i=0; i<SIZE; i++){ sm[i].TeleNo = 0; sm[i].flag = FALSE; sm[i].chain = -1;}}
int Hash(int x);
};
...
void Linear::search(int x){ 
	int start = Hash( x);
	for(int i=0; i< SIZE; i++){ 
		if(sm[start].TeleNo == x){ cout<< "\nThe required no. is found at location " << start << " in the hash table"; return;}
		else{start =(start+1)%SIZE;}
	}
	cout  << "There was no such element in the hash table";
}
void Linear::Display(){ 
cout << endl << "SrNo" << "    " << "TeleNo";
for(int i = 0; i <SIZE; i++){ if(sm[i].flag == 1){ cout << endl << i << "    " << sm[i].TeleNo;} 
				else{ cout << endl << i << "    " << "-----";}
			}
}		
/*----------------------CLASS_CHAINING---------------------------------------*/
...
void Chaining:: insert(int x){ 
	...
	start = Hash(x);
		...
		cout << i;
		j = start;
		while(sm[j].flag && i < SIZE){ j = (j+1)%SIZE; i = i+1;} if(i == SIZE){ cout  << "\n The HashTable is full";}
		sm[j].TeleNo =x;
		sm[j].flag = TRUE;
		if(start != j){sm[start].chain = j;}
}
void Chaining:: search(int x){ int i=0, start; start = Hash(x);
	/*Tracing the beginning of the chain*/
	while(i < SIZE && sm[start].flag && sm[start].TeleNo%SIZE != start){ start = (start+1)%SIZE; i++;}
	if(i == SIZE){cout<< "\nNo such element is found";}
/*locate the element in the chain*/
	while(start != -1){ 
		if(sm[start].TeleNo==x){cout << "\nThe required data is found at "<< start<<endl; return;} 
		start = sm[start].chain;
		}
	cout << "\nNo such element is found";
return;
...
void Chaining::Display(){ cout << endl << "SrNo" << "    " << "TeleNo" << "    "<< "chain";
	for(int i = 0; i <SIZE; i++){ if(sm[i].flag == TRUE){ cout << endl << i << "    " << sm[i].TeleNo << "    " << sm[i].chain;} 
				else{ cout << endl << i << "    " << "-----";}
			}
}		
/*----------------------MAIN-----------------------------------------------*/
